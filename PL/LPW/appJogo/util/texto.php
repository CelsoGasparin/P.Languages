...
class texto {
    ...
    public static function contemAcento($string){
        $qtdAc = 0;
        $iLen = strlen($string);
        $acentos = [
            'á','à','ã','â','Á','À','Ã','Â',
    ...
    public static function validCharac($string){
        ...
        foreach($texto as $key => $value) {
            if(str_contains($string,$value)){
                $valid += texto::repeatCharac($string,$value);
            }
        }
        if($acentos = texto::contemAcento($string)){
            $valid += $acentos*2;
        }
        // print "\n".$valid;
    ...
    public static function removerAcento(string $string){
        $iLen = strlen($string);
        $actualString = '';
        $acentos = [
            ['á','à','ã','â'],['Á','À','Ã','Â'],
        ...
        for($i=0; $i < $iLen; $i++){ 
            ...
            if(texto::validCharac($stringArray[$i])){
    ...
    public static function repeatCharac(string $string,$oi){
        $strLen = strlen($string);
        $strArray = [];
        $repeats = 0;
        for($i=0; $i < $strLen; $i++){ 
    ...
    public static function removerEspacos(string $string){
        $strLen = strlen($string);
        $strArray = [];
        $actualString = null;
        for($i=0; $i < $strLen; $i++){ 
    ...
    public static function clearString(string $string){
        $string = texto::removerAcento($string);
        $string = texto::removerEspacos($string);
